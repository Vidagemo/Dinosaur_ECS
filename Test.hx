...
class Test extends SampleApp{
    override function init() {
        ...
        bmp.clear(0x801ED618);
        bmp.line(0, 0, 63, 0, 0xFFFFFFFF);
        bmp.line(0, 0, 0, 63, 0xFF0000FF);
        bmp.line(0, 63, 63, 63, 0xFFFF0000);
        bmp.line(63, 0, 63, 63, 0xFF00FF00);
    
        var animationSupported = false;
        #if (flash || js || hldx || hlsdl)
        animationSupported = true;
        #end
        //trace(animationSupported);
        ...
        var cursors : Array<hxd.Cursor> = [Default,Button,Move,TextInput,Hide,
                                            Custom(new hxd.Cursor.CustomCursor([bmp],10,16,16)),
                                            Custom(new hxd.Cursor.CustomCursor(animatedFrames,10,16,16))];
        var pos = 3;
        for( c in cursors ) {
